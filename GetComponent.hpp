...
namespace OW {
	...
	inline uint64_t DecryptComponent(uintptr_t parent, uint8_t idx,uint64_t plus = 0)
{
	__try {
		unsigned __int64 var_v2 = (uint64_t)1 << (uint64_t)(idx & 0x3F);
		unsigned __int64 var_v3 = var_v2 - 1;
		unsigned __int64 var_v4 = idx & 0x3F;
		unsigned __int64 var_var_v1 = idx / 0x3F;
		uint64_t v9 = MEM->RPM<uint64_t>(parent + 8i64 * var_var_v1 + 272);
		uint64_t v10 = (var_v3 & v9) - (((var_v3 & v9) >> 1) & 0x5555555555555555i64);
		uint64_t var_qword = MEM->RPM<uint64_t>(MEM->GameImageBase + 0x372EE40);
		uint8_t var_byte = MEM->RPM<uint8_t>(MEM->GameImageBase + 0x3448026);

		uint64_t v15 = MEM->RPM<_QWORD>(parent + 128);
		uint64_t v19 = MEM->RPM<unsigned __int8>(var_var_v1 + parent + 304)
			+ ((0x101010101010101LL
		...
		v19 = MEM->RPM<_QWORD>(v15 + 8 * v19);
		v19 = __ROL8__(v19, 13);


		v19 = var_byte ^ v19;
		v19 -= 0x5CE8C80823ED9F9LL;
		v19 = __ROR8__(MEM->RPM<_QWORD>(var_qword + 203) ^ v19 ^ 0x9272AD14BAB97ABLL, 27)
			+ 0x6B81869D195CDA4CLL;
		v19 = eidolon(v19);
		v19 ^= 0x9200B34D13951004uLL;
		v19 += 0x5670E6B2337EB3C7LL;
		v19 = -(int)(__int64)((var_v2 & v9) >> var_v4) & (v19);
		return v19;
	}
	__except (1) { return 0; }
}
